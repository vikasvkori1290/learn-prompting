...
const iBase = {
    ...
    padding: '10px 12px 10px 38px', borderRadius: 6, fontSize: 14,
    background: T.white, color: T.black, fontFamily: "'Inter',sans-serif",
...
export default function RegisterPage() {
    ...
    const iStyle = (name, pr = false) => ({
        ...iBase,
        borderColor: focused === name ? T.blue : T.gray3,
        boxShadow: focused === name ? `0 0 0 3px rgba(33,150,243,0.1)` : 'none',
        border: `1px solid ${focused === name ? T.blue : T.gray3}`,
        ...(pr ? { paddingRight: 36 } : {}),
    });
    const iconWrap = { position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', color: T.gray2, pointerEvents: 'none' };
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '100px 16px 48px' }}>
                <div style={{ width: '100%', maxWidth: 400, background: T.white, border: `1px solid ${T.gray4}`, borderRadius: 12, padding: '36px 32px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
                    ...
                    <div style={{ textAlign: 'center', marginBottom: 26 }}>
                        ...
                        <h1 style={{ fontFamily: "'Space Grotesk','Inter',sans-serif", fontSize: 22, fontWeight: 700, color: T.black, marginBottom: 6 }}>Create your account</h1>
                        <p style={{ fontSize: 13, color: T.gray2 }}>Start your prompt training journey</p>
                    </div>

                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
                        <GoogleLogin onSuccess={handleGoogle} onError={() => toast.error('Google sign-up failed')} theme="outline" size="large" shape="rectangular" width="336" />
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '18px 0' }}>
                        <hr style={{ flex: 1, border: 'none', borderTop: `1px solid ${T.gray4}` }} />
                        <span style={{ fontSize: 12, color: T.gray2 }}>or use email</span>
                        <hr style={{ flex: 1, border: 'none', borderTop: `1px solid ${T.gray4}` }} />
                    </div>

                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {[
                            { name: 'name', type: 'text', placeholder: 'Full name', icon: <User size={14} />, ac: 'name' },
                            { name: 'email', type: 'email', placeholder: 'Email address', icon: <Mail size={14} />, ac: 'email' },
                        ].map(({ name, type, placeholder, icon, ac }) => (
                            <div key={name} style={{ position: 'relative' }}>
                                ...
                                <input type={type} name={name} placeholder={placeholder} value={form[name]} onChange={handleChange}
                                    ...
                                    style={iStyle(name)} autoComplete={ac} />
                            </div>
                        ))}

                        <div style={{ position: 'relative' }}>
                            <span style={iconWrap}><Lock size={14} /></span>
                            <input type={showPass ? 'text' : 'password'} name="password" placeholder="Password (min 6)" value={form.password} onChange={handleChange}
                                onFocus={() => setFocused('password')} onBlur={() => setFocused('')}
                                style={iStyle('password', true)} autoComplete="new-password" />
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type="password" name="confirm" placeholder="Confirm password" value={form.confirm} onChange={handleChange}
                                ...
                                style={iStyle('confirm')} autoComplete="new-password" />
                        </div>

                        <button type="submit" disabled={loading} style={{
                            width: '100%', padding: '11px', borderRadius: 6, marginTop: 6,
                            background: T.black, color: T.white, fontSize: 14, fontWeight: 700,
                            border: 'none', cursor: loading ? 'not-allowed' : 'pointer',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
                            opacity: loading ? 0.65 : 1, fontFamily: "'Inter',sans-serif", transition: 'background .18s',
                        }}
